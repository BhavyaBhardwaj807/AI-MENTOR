...
export default function StudentMeetings() {
  ...
  const upcoming = meetings.filter((m) => {
    const end_time = new Date(m.scheduled_at).getTime() + (m.duration_minutes || 60) * 60000;
    return end_time >= now.getTime();
  });
  ...
  function MeetingRow({ m }: { m: Meeting }) {
    ...
    const end_time = date.getTime() + (m.duration_minutes || 60) * 60000;
    const isLive = m.status === "live";
    const isUpcoming = end_time >= now.getTime() && m.status !== "ended";
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-4 items-start lg:items-center">
          ...
          <div className="lg:col-span-2 flex items-center justify-between lg:justify-end gap-3 w-full">
            ...
            {isLive ? (
              ...
              <span className="px-3 py-1.5 rounded bg-surface-container-high text-on-surface-variant text-[13px]">Waiting</span>
            ) : (
