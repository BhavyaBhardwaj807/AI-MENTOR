          ...
          <div className="flex-1 flex flex-col gap-2 p-3 overflow-y-auto">
            {loading ? (
              ...
              teachers.map((teacher) => (
                <button type="button" key={teacher.id} onClick={() => { setMessagesLoading(true); setSelectedTeacher(teacher); }} className={`w-full flex items-center gap-3 p-3 rounded-lg text-left transition-colors ${selectedTeacher?.id === teacher.id ? "bg-surface-container-high" : "bg-surface-container hover:bg-surface-container-high"}`}>
